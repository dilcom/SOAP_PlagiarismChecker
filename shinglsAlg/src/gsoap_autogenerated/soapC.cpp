...
SOAP_SOURCE_STAMP("@(#) soapC.cpp ver 2.8.10 2012-10-02 16:52:43 GMT")
...
{	(void)tag;
	...
	case SOAP_TYPE_t__result:
		return soap_out_t__result(soap, tag, id, (const result *)ptr, "t:result");
	case SOAP_TYPE_result:
	...
	case SOAP_TYPE_PointerTot__result:
		return soap_out_PointerTot__result(soap, tag, id, (result *const*)ptr, "t:result");
	case SOAP_TYPE_xsd__string:
...
SOAP_FMAC3 int SOAP_FMAC4 soap_out_t__result(struct soap *soap, const char *tag, int id, const result *a, const char *type)
{
	...
	if (soap_out_std__vectorTemplateOft__text(soap, "arrayOfTexts", -1, &(a->result::arrayOfTexts), ""))
		return soap->error;
	if (soap_out_t__status(soap, "errCode", -1, &(a->result::errCode), ""))
		return soap->error;
	return soap_element_end_out(soap, tag);
}
...
SOAP_FMAC3 result * SOAP_FMAC4 soap_in_t__result(struct soap *soap, const char *tag, result *a, const char *type)
{
	...
	if (!a)
		return NULL;
	if (soap->alloced)
	{	soap_default_t__result(soap, a);
		if (soap->clist->type != SOAP_TYPE_t__result)
		{	soap_revert(soap);
			*soap->id = '\0';
			return soap_in_t__result(soap, tag, a, type);
		}
	}
	...
	if (soap->body && !*soap->href)
	{
		...
		{	soap->error = SOAP_TAG_MISMATCH;
			...
			if (soap_flag_errCode1 && soap->error == SOAP_TAG_MISMATCH)
				if (soap_in_t__status(soap, "errCode", &(a->result::errCode), "t:status"))
				{	soap_flag_errCode1--;
					continue;
				}
			if (soap->error == SOAP_TAG_MISMATCH)
...
{
	register int id = soap_embed(soap, (void*)a, NULL, 0, tag, SOAP_TYPE_t__result);
...
void result::soap_default(struct soap *soap)
{
	...
	soap_default_t__status(soap, &this->result::errCode);
}
	...
	if (soap->body && !*soap->href)
	{
		...
		{	soap->error = SOAP_TAG_MISMATCH;
			if (soap_flag_res && soap->error == SOAP_TAG_MISMATCH)
				if (soap_in_PointerTot__result(soap, "res", &a->res, "t:result"))
				{	soap_flag_res--;
					continue;
				}
			if (soap->error == SOAP_TAG_MISMATCH)
