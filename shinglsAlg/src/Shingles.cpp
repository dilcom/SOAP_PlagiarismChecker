...
Shingle::Shingle(const t__text & inText)
{
    ...
    while ( posTxt < txtLength && !iswalpha( txt[posTxt] ))
        posTxt++;
    // very powerfull magic (at least for me about an year after I wrote it.
    // Following code transforms input text into text, that contains
    // only words more than MIN_WORD_LENGTH and without any other signs.
    // All words are separated by one " ".
    ...
    buff[posBuff] = L'\0';
    // magic ends, you can open your eyes
    // end of canonization
    if (posBuff != 0)
        wordCount += 1;
    ...
    unsigned int shingleCount = max(1, (int)(wordCount - DefaultValues::WORDS_EACH_SHINGLE + 1));
    ...
    // only MAX_SHINGLE_PER_TEXT (or less) minimal hashes shall pass
    nth_element(crcs, crcs + m_count, crcs + shingleCount);
    // copy hashes to m_data
    memcpy(m_data, crcs, m_count * sizeof(*m_data));
    time_t a;
    time(&a);
